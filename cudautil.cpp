...
{
    uchar s_temp[blockDim.x * blockDim.y * blockDim.z * 2];
    size_t pout =0;
    size_t pin = 1;
    ...
    if((x<width) && (y<height))
	{
	    s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z] = d_input[idx];
	    __syncthreads();
	    for (size_t offset = 1; offset < blockDim.z; offset *= 2)
	    {
	        pout = 1 - pout;
	        pin = 1 - pout;
	        if (threadIdx.z >= offset)
	        {
	           /*this is very frustrating indexing, should clean it later*/
	           s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z] = (uchar )fmaxf(s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z], s_temp[pin * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z - offset]);
	        } else {
	            s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	        ...
	        /*make sure to barrier again*/
	        __syncthreads();
	    }
		d_output[idx] = (s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z] > focalThresh) ? s_temp[pout * blockDim.x * blockDim.y * blockDim.z + 
	           (threadIdx.y * blockDim.x + threadIdx.x) * blockDim.z + 
	           threadIdx.z] : 0;
	}               
